...
(async () => {
  console.log('Applying borders to ALL sheets...\n');
  
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SPREADSHEET_ID });
  const allSheets = meta.data.sheets;
  
  console.log('Found sheets:', allSheets.map(s => s.properties.title).join(', '));
  console.log('');
  ...
  for (const sheet of allSheets) {
    ...
    try {
          ...
          requests: [{
            repeatCell: {
              ...
              cell: {
                userEnteredFormat: {
                  borders: {
                    top: { style: 'SOLID', color: { red: 0.15, green: 0.15, blue: 0.2 } },
                    bottom: { style: 'SOLID', color: { red: 0.15, green: 0.15, blue: 0.2 } },
                    left: { style: 'SOLID', color: { red: 0.15, green: 0.15, blue: 0.2 } },
                    right: { style: 'SOLID', color: { red: 0.15, green: 0.15, blue: 0.2 } }
                  }
                }
              },
      ...
      console.log('✓', sheetName, `(${(sheet.properties.gridProperties || {}).rowCount || 100} rows)`);
    } catch (e) {
      console.log('✗', sheetName + ':', e.message);
    }
  }
  
  console.log('\n✅ Done! Borders applied to ALL sheets.');
})();